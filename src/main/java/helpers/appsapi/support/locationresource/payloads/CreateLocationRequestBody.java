...
public class CreateLocationRequestBody {
    ...
    final public Map<CreateLocationCombination, Supplier<JSONObject>> MAP = new HashMap<>();
    final Countries randomCountry = Countries.getRandomCountry();
    ...
    public enum CreateLocationCombination {
        REQUIRED_FIELDS,
        ALL_FIELDS
    }
    ...
    private Supplier<JSONObject> requiredFields = () -> {
        ...
        createLocationBody.put(STATUS, LocationStatuses.ACTIVE.name());
        createLocationBody.put(TIMEZONE, Countries.getTimezoneForApi(randomCountry));

        createAddressBody.put(ADDRESS_LINE_1, FAKER.address().secondaryAddress());
        createAddressBody.put(CITY, FAKER.address().city());
        createAddressBody.put(COUNTRY, randomCountry.getCountry());
...
//        TODO state will be changed from String to List<String> in future to pick random element from that list
        createLocationBody.getJSONObject(ADDRESS).put(STATE_REGION, randomCountry.getState());

        return createLocationBody;
    };

    public JSONObject bodyBuilder(CreateLocationCombination mode) {
        return MAP.get(mode).get();
    }

    {
        MAP.put(CreateLocationCombination.REQUIRED_FIELDS, requiredFields);
        MAP.put(CreateLocationCombination.ALL_FIELDS, allFields);
    }
